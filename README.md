...
- expertise_regular, expertise_premium, expertise_special: represents whether the phlebotomist possesses the regular, premium and special level of expertise respectively in 

## 1.5 Simulating customer orders:

`create_orders_df(seed, coords, n_phleb, ratio=None)` takes in a seed, the tuple of coordinates generated by `generate_coords`, number of orders, and a list representing the ratio of services chosen by customers. For example, if ratio is set as [0.7, 0.2, 0.1], approximately 70%, 20% and 10% of all orders generated are from regular, premium, and special categories respectively. By default, the value of ratio is None and each order requested is randomly assigned a category from `get_skills()` under the assumption of a uniform distribution. It returns a dataframe of customer orders and its relevant information and saves it as a csv file under the Simulated Data folder.
...
- duration: represents the total time to complete a customer’s order based on the services chosen. Each service is assumed to take 15 minutes to complete. 

- price: price: represents the total revenue earned from completing a customer’s order based on the services chosen. Regular, premium, and special services cost 200, 300, and 400 respectively. 
